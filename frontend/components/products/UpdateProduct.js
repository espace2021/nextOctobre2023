...
async function getProductDetails(id){
  try {
 /*  const res = await fetch (`https://backend-ecommerce-jwt-2024.vercel.app/api/articles/${id}`);
...
const UpdateProduct = ({params,scategories}) => {
  ...
  const handleSubmit = async(event) => { 
    ...
    if (form.checkValidity() === true) {
     ...
     console.log(article)
   /* await ( fetch('https://backend-ecommerce-jwt-2024.vercel.app/api/articles/' + params.id, {
        method: 'PUT',
        body: article
    }))*/
    await editArticle(article)
      ...
      router.push("/admin/products/cardsList")
      })
      .catch(error=>{
      console.log(error)
